...
/* branch predictor type {nottaken|taken|perfect|bimod|2lev|(mod)alloy} */
...
/* (mod)Alloyed  predictor config (<l1size> <l2size> <hist_size> <global_hist_size> <xor>) */
static int alloy_nelt = 5;
static int alloy_config[5] =
  { /* l1size */16, /* l2size */1024, /* hist */8, /*glob hist  */ 8,  /* xor */FALSE};
...
"  Predictor `comb' combines a bimodal and a 2-level predictor.\n"
  ...
  opt_reg_int_list(odb, "-bpred:alloy",
		   ...
		   "(<l1size> <l2size> <hist_size> <global_hist_size> <xor>)",
		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  if (!mystricmp(pred_type, "perfect"))
  ...
  else if (!mystricmp(pred_type, "taken"))
    {
      ...
      pred = bpred_create(BPredTaken, 0, 0, 0, 0, 0,/*(mod) not used*/ 0, 0, 0, 0, 0);
    }
  else if (!mystricmp(pred_type, "nottaken"))
    {
      /* static predictor, taken */
      pred = bpred_create(BPredNotTaken, 0, 0, 0, 0, 0,/*(mod) not used*/ 0, 0, 0, 0, 0);
    }
  ...
  /* (mod) alloyed branch predictor config */
  else if (!mystricmp(pred_type, "alloy"))
    {
      /* Alloyed branch predictor, bpred_create() checks args */
      if (twolev_nelt != 4)
	fatal("bad alloyed pred config (<l1size> <l2size> <hist_size> <global_hist_size> <xor>)");
      if (btb_nelt != 2)
	fatal("bad btb config (<num_sets> <associativity>)");

      pred = bpred_create(BPredAlloy,
			  /* bimod table size */0,
			  /* 2lev l1 size */alloy_config[0],
			  /* 2lev l2 size */alloy_config[1],
			  /* meta table size */ 0,
			  /* history reg size */alloy_config[2],
			  /* glob history reg size */alloy_config[3],
			  /* history xor address */alloy_config[4],
			  /* btb sets */btb_config[0],
			  /* btb assoc */btb_config[1],
			  /* ret-addr stack size */ras_size);
    }
