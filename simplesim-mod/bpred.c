...
bpred_create(enum bpred_class class,	/* type of predictor to create */
	     ...
	     unsigned int glob_shift_width, /*(mod): global history register width */
	     ...
	     unsigned int retstack_size) /* num entries in ret-addr stack */
{
  ...
  case BPred2Level:
    ...
    /* (mod) Alloyed creation */
  case BPredAlloy:
    pred->dirpred.twolev = 
      bpred_dir_create(class, l1size, l2size, shift_width, glob_shift_width, xor);
  ...
  unsigned int xor)	    	/* history xor address flag */
{
  ...
  case BPredAlloy:
	...
	fatal("cannot allocate second level table");
      ...
      /* (mod) reserve space for one line table (GBHR) */
      pred_dir->config.two.shiftglob_reg = calloc(1, sizeof(int));
      if (!pred_dir->config.two.l2table)
	fatal("cannot allocate global shift register");
  ...
  FILE *stream)			/* output stream */
{
  ...
  case BPredAlloy:
    fprintf(stream,
      "pred_dir: %s: alloy: %d l1-sz, %d bits/ent, %d glob-BHR bits %s xor, %d l2-sz, direct-mapped\n",
      name, pred_dir->config.two.l1size, pred_dir->config.two.shift_width, 
      /* (mod) GHBR shift size */ pred_dir->config.two.shift_g_width,
      pred_dir->config.two.xor ? "" : "no", pred_dir->config.two.l2size);
    break;
		 ...
		 md_addr_t baddr)		/* branch address */
{
  ...
  switch (pred_dir->class) {
	...
	int l1index, l2index;
      ...
      break;
      /*(mod) Alloyed predictor */
    case BPredAlloy:
      {
	int l1index, l2index;
	int gbht_line;

        /* get the L1 (PaBHT index) */
	/* PaBHT index = @branch & (mask ok k 1s) */
        l1index = (baddr >> MD_BR_SHIFT) & (pred_dir->config.two.l1size - 1);
	...
	int p_g_width = pred_dir->config.two.shift_width + pred_dir->config.two.shift_g_width;
    	l2index =
      		l2index	/* PaBHT line */
		| (gbht_line << pred_dir->config.two.shift_width) /* GBHR line */
		| ((baddr >> MD_BR_SHIFT) << p_g_width);
	l2index = l2index & (pred_dir->config.two.l2size - 1);
	     ...
	     struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
  ...
  if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND) &&
      (pred->class == BPred2Level || pred->class == BPredComb || pred->class == BPredAlloy))
    {
      int l1index, shift_reg, g_shift_reg;
	...
	shift_reg & ((1 << pred->dirpred.twolev->config.two.shift_width) - 1);
      	...
      	if(pred->class == BPredAlloy)
      	{	
	      	/*(mod) update GBHR also */
		g_shift_reg = (pred->dirpred.twolev->config.two.shiftglob_reg[0] << 1) | !!taken;
		/* g_shift_reg & (mask of g 1s) */
		pred->dirpred.twolev->config.two.shiftglob_reg[0] =
		       	g_shift_reg & ((1 << pred->dirpred.twolev->config.two.shift_g_width) - 1); 
	}
    }
